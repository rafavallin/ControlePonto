...
export class TCTableComponent implements OnInit, OnChanges {
  ...
  @HostBinding('class.item-list') @Input() list: boolean;
  ...
  constructor() {
    ...
    this.rowAlign = 'left';
    this.headerColor = 'rgba(177, 111, 111, 0.8)';
    ...
    columnList.forEach((column: any) => {
      if (column.config.name) {
        filteredData = filteredData.filter((item: any) => {
          if (
            typeof item[column.config.name] === 'undefined' &&
            item[column.config.name] !== undefined
          ) {
            return item[column.config.name]
              .toString()
              .toLowerCase()
              .match(column.config.filtering.filterString.toLowerCase());
          } else {
            return true;
          }
        });
      ...
      columnList.forEach((column: any) => {
        if (column.config.filtering && column.config.name) {
          if (
            ...
            if (
              item[column.config.name]
                .toString()
                .toLowerCase()
                .startsWith(config.filtering.filterString.toLowerCase())
            ) {
              flag = true;
            }
          }
  ...
  onChangeTable(config: any, column: TCTableColComponent): any {
    ...
    if (column) {
      filteredData = this.changeFilter(
        this.data,
        column.config,
        this.columnList
      );
    ...
    this.rows = this.pagination
      ? this.changePage(this.page, this.itemsPerPage, sortedData)
      : sortedData;
  }
}
